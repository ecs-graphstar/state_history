...
public:
    ...
    StateHistory(flecs::world* w, size_t keyframe_every = 60, bool compress = true)
        ...
        tracked_component_ids = {
            ...
            w->component<RenderColor>().id(),
            w->component<FoodSprite>().id(),
        };
    ...
    void print_stats() {
        ...
        world->each<Position>([&entity_count](flecs::entity, Position&) {
            entity_count++;
        });

        size_t component_size = sizeof(Position);
        size_t naive_per_frame = entity_count * component_size;
        size_t naive_total = snapshots.size() * naive_per_frame;

        std::cout << "\n=== Rollback Memory Statistics ===\n";
        std::cout << "\n--- Configuration ---\n";
        std::cout << "Total snapshots: " << snapshots.size() << "\n";
        std::cout << "Keyframe interval: " << keyframe_interval << " frames\n";
        std::cout << "Entities tracked: " << entity_count << "\n";
        std::cout << "Compression: " << (enable_compression ? "ENABLED (zlib)" : "DISABLED") << "\n";
        std::cout << "Change tracking: ENABLED (observer-based)\n";

        std::cout << "\n--- Snapshot Breakdown ---\n";
        std::cout << "Keyframes: " << keyframe_count << " (" << total_component_states << " component states)\n";
        if (enable_compression) {
            std::cout << "  Uncompressed: " << keyframe_data_bytes << " bytes\n";
            std::cout << "  Compressed: " << keyframe_compressed_bytes << " bytes";
            if (keyframe_data_bytes > 0) {
                double ratio = (double)keyframe_data_bytes / keyframe_compressed_bytes;
                std::cout << " (" << ratio << "x)";
            }
            std::cout << "\n";
        } else {
            std::cout << "  Data size: " << keyframe_data_bytes << " bytes\n";
        }

        std::cout << "Diff frames: " << (snapshots.size() - keyframe_count) << "\n";
        std::cout << "  Diffs stored: " << total_diffs_stored << " (only changed components)\n";
        if (enable_compression) {
            std::cout << "  Uncompressed: " << diff_data_bytes << " bytes\n";
            std::cout << "  Compressed: " << diff_compressed_bytes << " bytes";
            if (diff_data_bytes > 0) {
                double ratio = (double)diff_data_bytes / diff_compressed_bytes;
                std::cout << " (" << ratio << "x)";
            }
            std::cout << "\n";
        } else {
            std::cout << "  Data size: " << diff_data_bytes << " bytes\n";
        }
        std::cout << "  Empty frames: " << empty_diff_frames << " (no changes detected)\n";

        std::cout << "\nEntity Lifecycle:\n";
        std::cout << "  Total created: " << total_entities_created << " entities\n";
        std::cout << "  Total destroyed: " << total_entities_destroyed << " entities\n";
        std::cout << "  Keyframe entity lists: " << keyframe_entities << " entity references\n";
        std::cout << "  Memory usage: " << entity_lifecycle_memory << " bytes (uncompressed)\n";

        std::cout << "\nRelationships: " << total_relationships << " total\n";
        std::cout << "  Keyframes:\n";
        if (enable_compression) {
            std::cout << "    Uncompressed: " << keyframe_rel_uncompressed << " bytes\n";
            std::cout << "    Compressed: " << keyframe_rel_compressed << " bytes";
            if (keyframe_rel_uncompressed > 0 && keyframe_rel_compressed > 0) {
                double ratio = (double)keyframe_rel_uncompressed / keyframe_rel_compressed;
                std::cout << " (" << ratio << "x)";
            }
            std::cout << "\n";
        } else {
            std::cout << "    Data size: " << keyframe_rel_uncompressed << " bytes\n";
        }

        std::cout << "  Diff frames:\n";
        if (enable_compression) {
            std::cout << "    Uncompressed: " << diff_rel_uncompressed << " bytes\n";
            std::cout << "    Compressed: " << diff_rel_compressed << " bytes";
            if (diff_rel_uncompressed > 0 && diff_rel_compressed > 0) {
                double ratio = (double)diff_rel_uncompressed / diff_rel_compressed;
                std::cout << " (" << ratio << "x)";
            }
            std::cout << "\n";
        } else {
            std::cout << "    Data size: " << diff_rel_uncompressed << " bytes\n";
        }

        std::cout << "\n--- Memory Usage ---\n";
        std::cout << "Snapshot structures: " << snapshot_overhead << " bytes";
        if (snapshots.size() > 0) {
            std::cout << " (" << (snapshot_overhead / snapshots.size()) << " bytes per snapshot)";
        }
        std::cout << "\n";

        size_t actual_component_bytes = enable_compression ?
            (keyframe_compressed_bytes + diff_compressed_bytes) :
            (keyframe_data_bytes + diff_data_bytes);
        size_t actual_relationship_bytes = enable_compression ?
            (keyframe_rel_compressed + diff_rel_compressed) :
            (keyframe_rel_uncompressed + diff_rel_uncompressed);
        size_t actual_data_bytes = actual_component_bytes + actual_relationship_bytes;

        std::cout << "Data storage: " << actual_data_bytes << " bytes\n";
        std::cout << "  Components: " << actual_component_bytes << " bytes\n";
        std::cout << "  Relationships: " << actual_relationship_bytes << " bytes\n";
        std::cout << "Entity lifecycle: " << entity_lifecycle_memory << " bytes\n";
        std::cout << "Previous frame cache: " << prev_state_memory << " bytes\n";
        std::cout << "\nTotal memory: " << total_memory << " bytes\n";

        // Breakdown of total memory
        size_t computed_total = snapshot_overhead + actual_data_bytes + entity_lifecycle_memory + prev_state_memory;
        std::cout << "  Breakdown:\n";
        std::cout << "    Snapshot overhead: " << snapshot_overhead << " bytes ("
                  << (100.0 * snapshot_overhead / total_memory) << "%)\n";
        std::cout << "    Component data: " << actual_component_bytes << " bytes ("
                  << (100.0 * actual_component_bytes / total_memory) << "%)\n";
        std::cout << "    Relationship data: " << actual_relationship_bytes << " bytes ("
                  << (100.0 * actual_relationship_bytes / total_memory) << "%)\n";
        std::cout << "    Entity lifecycle: " << entity_lifecycle_memory << " bytes ("
                  << (100.0 * entity_lifecycle_memory / total_memory) << "%)\n";
        std::cout << "    Prev frame cache: " << prev_state_memory << " bytes ("
                  << (100.0 * prev_state_memory / total_memory) << "%)\n";

        std::cout << "\n--- Comparison ---\n";

        // Calculate what naive full-state storage would use per frame
        size_t naive_bytes_per_frame = 0;
        for (size_t comp_size : tracked_component_sizes) {
            naive_bytes_per_frame += comp_size * tracked_entities.size();
        }
        // Add naive relationship storage (28 bytes per relationship, average across all frames)
        size_t avg_rels_per_frame = snapshots.empty() ? 0 : total_relationships / snapshots.size();
        size_t naive_rel_per_frame = avg_rels_per_frame * sizeof(RelationshipHeader);
        size_t naive_total_bytes = (naive_bytes_per_frame + naive_rel_per_frame) * snapshots.size();

        std::cout << "Naive full-state storage: " << naive_total_bytes << " bytes\n";
        std::cout << "  Components: " << (naive_bytes_per_frame * snapshots.size()) << " bytes\n";
        std::cout << "  Relationships: " << (naive_rel_per_frame * snapshots.size()) << " bytes\n";
        std::cout << "Actual storage: " << total_memory << " bytes\n";

        if (naive_total_bytes > 0 && total_memory < naive_total_bytes) {
            double savings_percent = 100.0 * (1.0 - (double)total_memory / naive_total_bytes);
            double compression_ratio = (double)naive_total_bytes / total_memory;
            std::cout << "Memory saved: " << (naive_total_bytes - total_memory)
                      << " bytes (" << savings_percent << "%)\n";
